...
def get_netflix_top1(country_code):
    ...
    top1_list = db.session.query(NetflixContent).join(NetflixTop10).\
        filter(NetflixTop10.country_code == country_code, NetflixTop10.week == '2021-040', NetflixTop10.rank == rank).all()
...
# 국가별 각 주차의 1위 영화, tv쇼 리스트 불러오기
@bp.route('/netflix/<string:country_code>/top1', methods=['GET'])
def get_netflix_top1_by_week(country_code):
    rank = 1

    week_list = db.session.query(NetflixTop10.week).filter(NetflixTop10.country_code == country_code).all()
    ...
    top1_list = db.session.query(NetflixContent).join(NetflixTop10).\
        filter(NetflixTop10.country_code == country_code, NetflixTop10.rank == rank).all()
    ...
    for week in week_list:
        ...
        filter(NetflixTop10.country_code == country_code, NetflixTop10.week == week, NetflixTop10.rank == rank).all()
