...
const GenreBar = React.memo(function GenreBar({ genreScore = [] }) {
  ...
  const genreScoreN = genreScores.map((genreInfo) => (
    <div className="ScoreItems" style={{ backgroundColor: genreInfo.color }}>
      {genreInfo.score}
    </div>
  ));
  const dGenreGraph = genreScores.map((genreInfo) => (
    <div
      className="GenreGrpahBar"
      style={{ width: '20px', height: genreInfo.score, backgroundColor: genreInfo.color, marginTop: '20px' }}
    ></div>
  ));
  const GenreName = genreScores.map((genreInfo) => (
    <div className="GenreGraphText" style={{ transform: 'rotate(-270deg)', whiteSpace: 'nowrap' }}>
      {genreInfo.genre}
    </div>
  ));
  return (
      ...
      <div className="GenreBox">
        <div className="GenreAScore">
          ...
          <DropdownButton id="dropdown-basic-button" title="Score">
            <Dropdown.Item eventkey="item1" onClick={ASCButton}>
              ASC
            </Dropdown.Item>
            <Dropdown.Item eventkey="item2" onClick={DESCButton}>
              DESC
            </Dropdown.Item>
            <Dropdown.Item eventkey="item2" onClick={NoneButton}>
              ABC
            </Dropdown.Item>
          </DropdownButton>

          <div className="addCell">
            <div className="GenreContainer">{genreTitle}</div>
